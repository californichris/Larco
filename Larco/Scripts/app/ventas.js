...
$(document).ready(function () {
    ...
    $.when(_getConfig('MergeOrdenes'), _getConfig('PlanosList')).done(function (json1, json2) {
        MERGE_CONFIG = json1, PLANO_CONFIG = $.page.appendConfigMaps(json2);
        $('div.catalog').Page({
...
function initializeCatalog(config) {
    ...
    var ventasCtlg = $(TABLE_SEL).Catalog({
        pageConfig: config,
        ...
        validate: function (tips) {
            ...
            if ($('#Stock').is(':checked')) {
                ...
                if ($('#OrdenId').val() == '') { //new
                    ...
                    var data = getSelectedRowData($(TABLE_SEL).DataTable());
...
function editProdOrden(entity) {
    var prevData = getSelectedRowData($(TABLE_SEL).DataTable());
...
function executeSaveOrdenTransaction(entities) {
    $.when(executeTransaction(entities)).done(function (json) {
        if (json.ErrorMsg == SUCCESS) {
            ...
            showError($(DIALOG_SEL + " p.validateTips"), 'Ya existe una Orden de Trabajo con este numero.');
        } else {
            showError($(DIALOG_SEL + 'p.validateTips'), 'No fue posible crear la Orden de Trabajo.');
        }
    });
}
...
function executeEditOrdenTransaction(entities) {
    $.when(executeTransaction(entities)).done(function (json) {
        if (json.ErrorMsg == SUCCESS) {
            ...
            showError($(DIALOG_SEL + ' p.validateTips'), 'No fue posible actualizar la Orden de Trabajo.');
        }
    });
}
...
function clearAdditionalInfoTables() {
    ...
    $(DIALOG_SEL + ' td.dataTables_empty').remove();
}

function appendAdditionalInfoSection() {
    var table = $(DIALOG_SEL + ' #tabs-1 table.table-style');
...
function createEventHandlers() {
    ...
    $('#PN_Id').attr('title','Por favor capture al menos 3 caracteres para desplegar la lista de planos').autocomplete({
        minLength: 1,
        source: function (request, response) {
            ...
            $.when(getTopPlanos(request.term)).done(function (json) {
                response(json.aaData);
            });
...
function getTopPlanos(planoNumero) {
    //var config = $(TABLE_SEL).Catalog('getCatalogOptions').pageConfig;
    var colIdxs = getGridColsIndexes(PLANO_CONFIG);
    var order = [{ column: colIdxs.label, dir: 'asc' }];
    var where = { label: planoNumero };

    return getTopRecords(PLANO_CONFIG, 50, order, where);
}
            ...
            if ($('#OrdenId').val() == '' && _value.length == 6) {//only in new
                ...
                if (numbers.length >= 1) {
                    $.when(getLatestOrderByClient(numbers[1])).done(function (json) {
                        if (json.aaData && json.aaData.length > 0) {
                            populateDialog(json.aaData[0], DIALOG_SEL);
